...
export function ProductDetail({ 
  ...
  const selectedVariantText = Object.keys(selectedVariants).length > 0 
    ? ` (${Object.entries(selectedVariants).map(([k, v]) => `${k}: ${v}`).join(', ')})`
    : ''
  
  const whatsappMessage = `Hi, I'm interested in ${String(product.name)}${selectedVariantText}. Can you provide more details?`
            ...
            <div className="bg-white rounded-lg p-6 space-y-6" style={{ boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)' }}>
              ...
              <div>
                <a
                  ...
                  className="w-full flex items-center justify-center gap-2 rounded-xl bg-[#25D366] px-5 py-3 text-center font-semibold text-white shadow-md transition-all duration-300 hover:bg-[#20BA5A] active:scale-[0.98] mb-4"
                >
                  <svg 
                    width="20" 
                    height="20" 
