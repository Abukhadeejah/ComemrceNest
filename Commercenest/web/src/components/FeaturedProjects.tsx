...
export default async function FeaturedProjects() {
  ...
  const { data } = tenantId ? await fetchPublishedProjects(tenantId) : { data: [] as any[] }
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-12">
      <div className="flex items-end justify-between">
        ...
        <a className="text-sm underline" href="/portfolio">View all</a>
      </div>
      <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((p) => (
          <a key={p.id} href={`/portfolio/${p.slug}`} className="group block overflow-hidden rounded border">
