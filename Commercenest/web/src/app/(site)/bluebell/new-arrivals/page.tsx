...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return getPageMetadata('New Arrivals', 'See the latest additions to Bluebell Interiors')
}
...
export default async function NewArrivalsPage({ searchParams }: NewArrivalsPageProps) {
  ...
  const products = await getProducts({
    tenantId,
    sort: 'updated_at', // Default sort is already updated_at desc
    page: parseInt(params.page || '1'),
    limit: 12
  })

  console.log('[BLUEBELL_NEW_ARRIVALS_PAGE] Fetched products for tenant:', tenantId, 'Count:', products.length)
      ...
      <div className="bg-gray-50 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <nav className="flex items-center space-x-2 text-sm">
            ...
            <span className="text-[#01589D] font-semibold">New Arrivals</span>
          </nav>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl md:text-2xl text-[#4E302E] max-w-3xl mx-auto font-light leading-relaxed">
            Discover our latest fabrics and interior design products
          </p>
        </div>
