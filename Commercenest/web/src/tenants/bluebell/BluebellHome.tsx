...
export default function BluebellHome() {
  ...
  useEffect(() => {
    const id = setInterval(() => setTIndex((i) => (i + 1) % 3), 6000)
  ...
  return (
    <main className="p-0">
      ...
      <section className="py-24 bg-gradient-to-br from-white via-gray-50 to-white relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16">
            <h2 className={`${playfair.className} text-5xl md:text-7xl font-black text-primary mb-4 tracking-tight`}>What Our Clients Say</h2>
            <div className="w-20 md:w-28 h-1 bg-mustard mx-auto mb-6"></div>
            <p className="text-brown text-xl md:text-2xl font-normal leading-relaxed max-w-4xl mx-auto">
          ...
          <div className="relative overflow-hidden">
            <div className="flex transition-transform duration-700 ease-out" style={{ transform: `translateX(-${tIndex * 100}%)` }}>
              {[
                { initials: 'AK', name: 'Anita Kapoor', role: 'Interior Designer', quote: 'Bluebell Fabrics elevated our project with impeccable textures and colours.' },
                { initials: 'RS', name: 'Rahul Shah', role: 'Homeowner', quote: 'Premium feel and on-time delivery. Our living room looks stunning.' },
                { initials: 'ND', name: 'Nisha Desai', role: 'Studio Owner', quote: 'Reliable partner for every collection—great variety and quality.' },
              ].map((t, idx) => (
                <div key={t.name} className="w-full md:w-1/2 lg:w-1/3 flex-shrink-0 px-2 md:px-4">
                  <div className={`bg-white p-8 md:p-10 rounded-2xl shadow-xl border-l-4 border-crimson relative overflow-hidden transition-all duration-700 ${loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`} style={{ transitionDelay: `${idx * 120}ms` }}>
                    ...
                    <div className="flex items-center">
                      <div className="w-12 h-12 md:w-14 md:h-14 bg-gradient-to-br from-brown to-primary rounded-full flex items-center justify-center text-white font-bold text-base mr-4 shadow-lg">
                        {t.initials}
                      </div>
                      <div>
                        <h4 className="font-bold text-primary text-base md:text-lg">{t.name}</h4>
                        <p className="text-brown font-medium text-sm md:text-base">{t.role}</p>
                        <div className="mt-1 text-mustard">★★★★★</div>
                      </div>
                    </div>
                  </div>
                </div>
