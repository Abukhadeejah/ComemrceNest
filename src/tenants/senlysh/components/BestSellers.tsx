            ...
            <div key={product.name} className="group relative flex-shrink-0 w-full animate-fade-in-up px-2"
              ...
              <Link href={product.url} className="block">
                ...
                <Image
                  ...
                  onError={(e) => {
                    console.warn(`BestSeller image failed to load: ${product.image}. Product: ${product.name}`);
                    // Use a specific fallback that indicates missing product image
                ...
                {/* View Details CTA (non-anchor to avoid nested <a>) */}
                <div
                  className="absolute bottom-16 left-2 right-2 bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-semibold shadow-lg hover:bg-blue-700 transition-all duration-200 hover:scale-105 hover:shadow-xl opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 flex items-center justify-center gap-2 pointer-events-none"
                  aria-hidden
                >
