...
export default async function CustomerDetailPage({ params }: PageProps) {
	...
	const customerEmail = customerRow?.email || customerId

	// Aggregate customer data from orders by customer_id or fallback email
	...
	return (
		<div className="p-6 space-y-6">
			<div className="flex items-center justify-between">
				<div>
					<h1 className="text-2xl font-semibold text-gray-900">{customerRow?.first_name || customerEmail}</h1>
							...
							{(orders || []).map(o => (
								<tr key={o.id}>
									<td className="px-4 py-2 text-sm text-blue-700">
										<Link href={`/admin/order-details/${o.id}`} className="hover:text-blue-900 hover:underline">
											{o.order_number}
										</Link>
									</td>
									...
									<td className="px-4 py-2 text-sm">
										<div className="flex items-center gap-3">
											<Link href={`/admin/order-details/${o.id}`} className="text-blue-600 hover:text-blue-800 hover:underline">
												View Details
											</Link>
											<a
