...
export default function CustomerAddressForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Client-side validation (Senlysh locale)
    const nextFieldErrors: Partial<Record<keyof AddressFormData, string>> = {}
    if (!formData.full_name.trim()) nextFieldErrors.full_name = 'Full name is required'
    if (!/^\d{10}$/.test(formData.phone.replace(/\D/g, ''))) nextFieldErrors.phone = 'Enter a valid 10-digit mobile number'
    if (!formData.address_line_1.trim()) nextFieldErrors.address_line_1 = 'Address line 1 is required'
    if (!formData.city.trim()) nextFieldErrors.city = 'City is required'
    if (!formData.state.trim()) nextFieldErrors.state = 'State is required'
    if (!/^\d{6}$/.test(formData.postal_code.replace(/\D/g, ''))) nextFieldErrors.postal_code = 'Enter a valid 6-digit pincode'
    if (!formData.country.trim()) nextFieldErrors.country = 'Country is required'
    ...
    try {
      ...
      if (response.ok) {
        // Optimistic UX: close modal quickly, then refresh list
        resetForm()
        setShowForm(false)
        await loadAddresses()
      } else {
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white w-full max-w-2xl mx-4 rounded-xl shadow-xl border border-pink-100">
            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">{editingAddress ? 'Edit Address' : 'Add New Address'}</h3>
              <button type="button" onClick={handleCancel} className="text-gray-500 hover:text-gray-700">✕</button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  required
                />
                  {fieldErrors.address_line_1 && <p className="mt-1 text-xs text-red-600">{fieldErrors.address_line_1}</p>}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  required
                />
                  {fieldErrors.postal_code && <p className="mt-1 text-xs text-red-600">{fieldErrors.postal_code}</p>}
              </div>
            ...
            <div className="flex gap-4 justify-end">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="bg-pink-600 text-white px-6 py-2 rounded-md hover:bg-pink-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
